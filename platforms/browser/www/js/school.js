...
var schoolPage = {
  ...
  preparePage: function() {
    ...
    $(document).on("click", "#save_school_btn", function(event) {
      ...
      client.chosenSchool = name; //TODO: create a school object
      var street = $("#add_school_street").val();
      var stage = $("#add_school_stage").val();
      var zyklus = $("#add_school_zyklus").val();
      var formData = "label=" + name;
      if (street != undefined && street.length > 0) { 
        formData += "&address=" + street;
      }
      const addSchoolUrl = "http://localhost:8081/lp21/school";
      console.log(addSchoolUrl + " POST " + formData);
      /* in body as json does not work cross site due to https://stackoverflow.com/questions/21783079/ajax-in-chrome-sending-options-instead-of-get-post-put-delete
      $.ajax({
        url: addSchoolUrl, 
        type: "POST",
        data: formData,
        ...
        success: function(){
          // apply id of new school generated by DB
          client.chosenSchoolId = newId.school_id;        
          console.log("got school " + name + " .id=" + client.chosenSchoolId);
        }
      });
      */
      // url encoded TODO: escape???
      $.post(addSchoolUrl, formData, function(newId) {
        // apply id of new school generated by DB
        client.chosenSchoolId = newId.school_id;        
        console.log("got school " + name + " .id=" + client.chosenSchoolId);
      });
